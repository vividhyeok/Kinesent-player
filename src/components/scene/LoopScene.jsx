...
function LoopScene({ scene }) {
  ...
  return (
    <div className="relative flex h-full w-full items-center justify-center bg-slate-950">
      ...
      {isPlaybackBlocked ? (
        <div
          className="absolute bottom-5 left-5 max-w-sm rounded-[1.5rem] border border-amber-300/40 bg-slate-950/82 px-4 py-4 text-sm text-amber-50 shadow-2xl shadow-slate-950/30 backdrop-blur"
          data-stage-interactive="true"
        >
          <p className="font-semibold">자동 재생이 차단됐습니다</p>
          <p className="mt-2 leading-6 text-amber-100/90">
            브라우저 정책 때문에 영상이 바로 재생되지 않았습니다. 아래 버튼을 눌러 수동으로
            재생해 주세요.
          </p>
          <button
